...
X <- model.matrix(~ . - 1, data = stroke_train)  # Remove intercept term
...
The Lasso Regression model performs the best among the three models. It has the lowest MSE, indicating that the predicted probabilities from the Lasso model are closest to the actual outcomes on the training set. This suggests that Lasso Regression, which includes a penalty term that encourages the model to reduce the number of coefficients, effectively minimizes overfitting and improves the model's predictive accuracy. Ridge Regression also improves upon the basic Logistic Regression model but does not reach the predictive accuracy of the Lasso model. Logistic Regression, without any regularization, has the highest MSE, suggesting it might be overfitting to the training data.

Now, I will plot the residuals:

```{r}
residuals_data <- data.frame(
  Observed = c(observed_vals_lr, observed_vals_lasso, observed_vals_ridge),
  Residuals = c(res_lr, res_lasso, res_ridge),
...
ggplot(residuals_data, aes(x = Observed, y = Residuals, color = Model)) +
  ...
  labs(title = "Residuals Plot for Logistic, Lasso, and Ridge Regression Models",
       x = "Observed Values",
       y = "Residuals") +
  theme_minimal() +
...
Moving forward, lets use the Lasso Model
...
# Now, retry the prediction
predicted_probs <- predict(lasso_model, newx = X_test, s = optimal_lambda, type = "response")

# Create a data frame for visualization
predictions <- data.frame(Probability = predicted_probs)


ggplot(predictions, aes(x = predicted_probs)) +
